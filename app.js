...
const STORAGE_KEY='hmi-training-v10-state';
function saveSim(){
  try{
...
function resetSimulation(){
  try{['hmi-training-v10-state','hmi-training-v092-state','hmi-training-v091-state','hmi-training-v09-state','hmi-training-v08-state'].forEach(k=>localStorage.removeItem(k));}catch(e){}
  location.reload();
}
...
function handleAction(button){const action=button.dataset.action;
  ...
  if(action==='launch-tia'||action==='launch-eplan'){toast('External engineering application intentionally not launched in training');return;} if(action==='project-info'){const e=document.getElementById('adminInfo');if(e)e.textContent='Project: HMI Training Simulator v1.0 · synchronized sequence diagnostics · interactive simulated PLC cell · no PLC connection.';return;} if(action==='clean-screen'){toast('Clean screen simulated');return;} if(action==='end-wincc'){toast('Exit disabled in training');return;}
