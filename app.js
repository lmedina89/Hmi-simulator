...
const STORAGE_KEY='hmi-training-v07-state';
function saveSim(){
  try{
    localStorage.setItem(STORAGE_KEY,JSON.stringify({robots,state:{
      ...
      seqStep:state.seqStep,sequences:state.sequences,processModes:state.processModes,interfacePage:state.interfacePage,
...
function robotReadyForAuto(r){return !r.fault&&r.masi&&r.drives&&r.pf0&&!r.maintenance;}
function robotFault(id,text){const r=robots[id];r.fault=true;r.running=false;r.fk=false;r.auto=false;addAlarm(`TRAINING: ${id} ${text}`,'fault');saveSim();}
function robotStatusCommand(id,kind){
  const r=robots[id]; if(!r)return;
  state.selectedRobot=id; state.selectedStation=r.station;
  if(kind==='masi'){
    if(r.maintenance){addAlarm(`TRAINING: ${id} MaSi rejected — maintenance active.`,'fault');toast('MaSi inhibited');return;}
    if(!state.robotFaultAcknowledged && r.fault){addAlarm(`TRAINING: ${id} safety reset rejected — acknowledge fault first.`,'fault');toast('Acknowledge fault first');return;}
  ...
  if(kind==='drives'){
    ...
    r.drives=true; r.fault=false; toast(`${id}: Drives enabled`);
  }
  if(kind==='pf0'){
...
function acknowledge(){const a=state.alarms.find(x=>!x.ack);if(a)a.ack=true;state.robotFaultAcknowledged=true;saveSim();updatePersistentUI();toast(a?'Alarm acknowledged':'No unacknowledged alarms');}
...
loadSim();tick();setInterval(tick,1000);render('main',{noHistory:true});
