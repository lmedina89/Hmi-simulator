...
const STORAGE_KEY='hmi-training-v092-state';
function saveSim(){
  try{
...
function loadSim(){
  try{
    ...
    // One-time migration from v0.8.x so users keep their training state.
    if(!saved){
...
function renderAdminCellPanel(){
  ...
  if(!state.media.air11)injected.push('Air pressure = 0');
...
function handleAction(button){const action=button.dataset.action;
  ...
  if(action==='trainer-air'){state.media.air11=!state.media.air11;if(state.media.air11)clearCellFault('AIR');saveSim();renderAdminCellPanel();return;}
  ...
  if(action==='launch-tia'||action==='launch-eplan'){toast('External engineering application intentionally not launched in training');return;} if(action==='project-info'){const e=document.getElementById('adminInfo');if(e)e.textContent='Project: HMI Training Simulator v0.9.1 · synchronized sequence diagnostics · interactive simulated PLC cell · no PLC connection.';return;} if(action==='clean-screen'){toast('Clean screen simulated');return;} if(action==='end-wincc'){toast('Exit disabled in training');return;}
