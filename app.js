...
function reconcileCurrentAlarms(){
  ...
  state.alarms.forEach(a=>{
    ...
    else if(key==='CELL_K100'&&state.controlVoltage)markAlarmCleared(a);
...
function renderStationStatus(){
  ...
  if(isTrainingStation(state.selectedStation)){const c=cellState(),r=trainingRobot(),events=c.eventLog.slice(0,3);el.innerHTML=`<div class="section-title">Live PLC status</div><div><b>K100</b> ${state.controlVoltage?'1':'0'} · <b>Safety</b> ${c.safetyDoorClosed&&c.safetyAcknowledged?'1':'0'} · <b>Air Cmd</b> ${state.media.air11?'1':'0'} · <b>Pressure</b> ${c.airPressureOk?'1':'0'}</div><div><b>Part</b> ${c.partPresent?'1':'0'} · <b>Clamp cmd</b> ${c.clampCommand?'1':'0'} · <b>MM12b</b> ${state.fixture.MM12b?'1':'0'}</div><div><b>Robot release</b> ${c.robotRelease?'1':'0'} · <b>Auto</b> ${r.auto?'1':'0'} · <b>FK</b> ${r.fk?'1':'0'}</div>${events.map(e=>`<div class="event-mini">${new Date(e.time).toLocaleTimeString()} ${e.text}</div>`).join('')}`;return;}
...
function initScreen(name){
  ...
  if(name==='preselection'){document.querySelectorAll('.mode-panel').forEach(el=>el.innerHTML=makeModePanel(el.dataset.cell));const airFb=document.getElementById('air11Feedback');if(airFb)airFb.textContent=`Pressure feedback: ${cellState().airPressureOk?'OK':'LOW'}`;const cv=document.getElementById('controlVoltage');if(cv){cv.textContent=state.controlVoltage?'On':'Off';cv.classList.toggle('active-green',state.controlVoltage);cv.dataset.action='control-voltage';}document.querySelectorAll('.utility-card .segmented').forEach(g=>g.querySelectorAll('button').forEach(b=>b.dataset.action='segmented-choice'));}
