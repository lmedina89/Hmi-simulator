...
const state={
  currentScreen:'main',history:[],selectedStation:'115000 UQ1',selectedRobot:'115010R01',viewerStation:'115020 V01',robotOverview:1,processOverview:1,snmpPage:0,selectedSnmp:null,
  ...
  trainingCell:{station:'115020 V01',robot:'115010R01',safetyDoorClosed:true,safetyAcknowledged:true,partPresent:true,clampCommand:false,robotRelease:false,interfaceRelease:true,failClampFeedback:false,airPressureOk:true,cycleActive:false,sequenceStep:1,stepChangedAt:0,cycleStartedAt:0,lastCycleTime:0,cycleCount:0,faultCode:null,faultText:'',challenge:null,incorrectActions:0,eventLog:[]},
  ...
  alarms:[
    {id:'R115010',key:'ROBOT_115010R01',severity:'fault',ack:false,active:true,time:Date.now(),text:'TRAINING: 115010R01 robot fault — drives not enabled.'}
  ]
 };

const STORAGE_KEY='hmi-training-v101-state';
function saveSim(){
  try{
    localStorage.setItem(STORAGE_KEY,JSON.stringify({robots,state:{
      selectedStation:state.selectedStation,selectedRobot:state.selectedRobot,viewerStation:state.viewerStation,robotOverview:state.robotOverview,
...
function loadSim(){
  try{
    ...
    // One-time migration from earlier builds so users keep their training state.
    if(!saved){
      const prior10=JSON.parse(localStorage.getItem('hmi-training-v10-state')||'null');
      if(prior10)saved=prior10;
    }
    if(!saved){
      const prior092=JSON.parse(localStorage.getItem('hmi-training-v092-state')||'null');
      if(prior092)saved=prior092;
    }
    if(!saved){
      const prior091=JSON.parse(localStorage.getItem('hmi-training-v091-state')||'null');
      if(prior091)saved=prior091;
    }
    if(!saved){
      const prior=JSON.parse(localStorage.getItem('hmi-training-v09-state')||'null');
      if(prior)saved=prior;
    }
    if(!saved){
      const legacy=JSON.parse(localStorage.getItem('hmi-training-v08-state')||'null');
    ...
    if(typeof state.trainingCell.airPressureOk!=='boolean')state.trainingCell.airPressureOk=true;
    state.alarms=(state.alarms||[]).map(a=>({active:a.active!==false,ack:!!a.ack,time:a.time||Date.now(),...a})).filter(a=>a.key!=='LUBE'&&!/Lubrication level low/i.test(a.text||''));
    state.alarms.forEach(a=>{if(a.key==='ROBOT_115010')a.key='ROBOT_115010R01';});
    // v0.9.1 migration: operator STOP messages are history events, not current alarms.
    state.alarms.forEach(a=>{if(/\bstopped\.?$/i.test(a.text||'')||/operator stop/i.test(a.text||'')){a.active=false;a.ack=true;a.cleared=a.cleared||Date.now();}});
    reconcileCurrentAlarms();
    saveSim();
  }catch(e){}
}
function resetSimulation(){
  try{['hmi-training-v101-state','hmi-training-v10-state','hmi-training-v092-state','hmi-training-v091-state','hmi-training-v09-state','hmi-training-v08-state'].forEach(k=>localStorage.removeItem(k));}catch(e){}
  location.reload();
}
function robotReadyForAuto(r){return !!r&&!r.fault&&r.masi&&r.drives&&r.pf0&&!r.maintenance;}
function robotAutoInhibitReason(r){
  if(!r)return 'robot state unavailable';
  if(r.maintenance)return 'Maintenance mode active';
  if(r.fault)return 'robot fault active';
  if(!r.masi)return 'MaSi / safety release OFF';
  if(!r.drives)return 'Drives OFF';
  if(!r.pf0)return 'PF0 / Home not reached';
  return null;
}
function robotStartInhibitReason(r){return robotAutoInhibitReason(r)||(!r.auto?'Auto not selected':null);}
function k100InhibitReason(){const c=cellState(),r=trainingRobot();if(!c.safetyDoorClosed)return 'safety door open';if(!c.safetyAcknowledged)return 'safety acknowledgement missing';if(r.maintenance)return 'robot Maintenance mode active';if(!r.masi)return 'MaSi / safety release OFF';return null;}
function k100Active(){return !k100InhibitReason();}
function robotFault(id,text){
  const r=robots[id]; if(!r)return;
  r.fault=true;r.running=false;r.fk=false;r.auto=false;
  state.robotFaultAcknowledged=false;
...
function robotStatusCommand(id,kind){
  ...
  if(kind==='masi'){
    if(r.masi){
      const c=cellState(),wasActive=id===state.trainingCell.robot&&(r.running||c.cycleActive);
      r.masi=false;r.drives=false;r.auto=false;r.running=false;
      if(id===state.trainingCell.robot){
        c.robotRelease=false;
        if(wasActive){c.cycleActive=false;state.stationRunning[c.station]=false;setCellFault('K100','Sequence stopped — MaSi / safety release removed.');recordEvent('MaSi / safety release removed during active operation');}
        else recordEvent('MaSi / safety release removed');
      }
      toast(`${id}: MaSi / safety release OFF`);saveSim();renderRobotRows();refreshCurrentScreen();updatePersistentUI();return;
    }
...
function markAlarmCleared(a){if(!a||!a.active)return;a.active=false;a.ack=true;a.cleared=a.cleared||Date.now();}
function reconcileCurrentAlarms(){
  const c=cellState(),r=trainingRobot();
  state.alarms=(state.alarms||[]).filter(a=>a.key!=='LUBE'&&!/Lubrication level low/i.test(a.text||''));
  state.alarms.forEach(a=>{
    if(!a.active)return;
    const key=a.key||'';
    const robotMain=key.match(/^ROBOT_([^_]+)$/);
    if(robotMain){const rr=robots[robotMain[1]];if(rr&&!rr.fault)markAlarmCleared(a);return;}
    const autoReject=key.match(/^ROBOT_([^_]+)_AUTO_REJECT$/);
    if(autoReject){const rr=robots[autoReject[1]];if(rr&&robotReadyForAuto(rr))markAlarmCleared(a);return;}
    const startReject=key.match(/^ROBOT_([^_]+)_START_REJECT$/);
    if(startReject){const rr=robots[startReject[1]];if(rr&&robotReadyForAuto(rr)&&rr.auto)markAlarmCleared(a);return;}
    if(key==='CELL_STOP'){markAlarmCleared(a);return;}
    if(key==='CELL_AIR'&&c.airPressureOk)markAlarmCleared(a);
    else if(key==='CELL_AIR_CMD'&&state.media.air11)markAlarmCleared(a);
    else if(key==='CELL_PART'&&c.partPresent)markAlarmCleared(a);
    else if(key==='CELL_CLAMP'&&state.fixture.MM12b)markAlarmCleared(a);
    else if(key==='CELL_INTERFACE'&&c.interfaceRelease)markAlarmCleared(a);
    else if(key==='CELL_SAFETY'&&c.safetyDoorClosed)markAlarmCleared(a);
    else if(key==='CELL_SAFETY_ACK'&&c.safetyDoorClosed&&c.safetyAcknowledged)markAlarmCleared(a);
    else if(key==='CELL_ROBOT_READY'&&robotReadyForAuto(r)&&r.auto)markAlarmCleared(a);
    else if(key==='CELL_K100'&&k100Active())markAlarmCleared(a);
    else if(key==='CELL_MODE'&&(state.stationMode[c.station]||'automatic')==='automatic')markAlarmCleared(a);
  });
}
function activeAlarm(){reconcileCurrentAlarms();return state.alarms.find(a=>a.active&&!a.ack)||state.alarms.find(a=>a.active)||null;}
function updatePersistentUI(){
  reconcileCurrentAlarms();
  // Sanitize legacy/static messages every render. STOP is an event/history item, not an active fault.
  state.alarms=state.alarms.filter(a=>a.key!=='LUBE'&&!/Lubrication level low/i.test(a.text||''));
  state.alarms.forEach(a=>{
    if(/\bstopped\.?$/i.test(a.text||'')||/operator stop/i.test(a.text||'')){
      a.active=false;a.ack=true;a.cleared=a.cleared||Date.now();
    }
  });
  ...
  const k100=k100Active(),k100Lamp=document.querySelector('.status-light');
  if(k100Lamp){const reason=k100InhibitReason();k100Lamp.classList.toggle('k100-off',!k100);k100Lamp.classList.toggle('k100-on',k100);k100Lamp.title=k100?'K100 safety circuit made':`K100 not made — ${reason}`;}
...
function addHistoryEvent(text,severity='warning',key=`HIST_${Date.now()}`){state.alarms.unshift({id:`H${Date.now()}${Math.random().toString(16).slice(2,5)}`,key,severity,ack:true,active:false,time:Date.now(),cleared:Date.now(),text});saveSim();updatePersistentUI();}
...
function airPressureAvailable(){const c=cellState();return !!state.media.air11&&c.airPressureOk!==false;}
...
function trainingPrerequisiteFailure(){
  ...
  const robotReason=robotStartInhibitReason(r);
  if(robotReason)return `${c.robot} ${robotReason}`;
  return null;
}
function startTrainingCycle(){
  ...
  c.faultCode=null;c.faultText='';c.cycleActive=true;c.sequenceStep=1;state.seqStep=1;c.stepChangedAt=Date.now();c.cycleStartedAt=Date.now();c.clampCommand=false;c.robotRelease=false;
...
function stopTrainingCycle(reason='operator stop'){
  ...
  addHistoryEvent(`TRAINING: ${c.station} stopped — ${reason}.`,'warning','CELL_STOP');recordEvent(`Cycle stopped: ${reason}`);saveSim();refreshCurrentScreen();
}
...
function simulationTick(){
  ...
  if(!k100Active()){c.cycleActive=false;setCellFault('K100','Sequence stopped — K100 safety circuit not made.');return;}
  switch(c.sequenceStep){
    case 1:{
      const mode=state.stationMode[c.station]||'automatic';
      if(mode!=='automatic'){setCellFault('MODE','Sequence waiting — Automatic mode not selected.');return;}
      if(!c.safetyAcknowledged){setCellFault('SAFETY_ACK','Sequence waiting — safety acknowledgement missing.');return;}
      const robotReason=robotStartInhibitReason(r);
      if(robotReason){setCellFault('ROBOT_READY',`Sequence waiting — ${c.robot}: ${robotReason}.`);return;}
      if(!state.media.air11){setCellFault('AIR_CMD','Sequence waiting — compressed air enable is OFF.');return;}
      if(!c.airPressureOk){setCellFault('AIR','Sequence waiting — air pressure feedback not ready.');return;}
      ['MODE','SAFETY_ACK','ROBOT_READY','AIR_CMD','AIR'].forEach(code=>clearAlarm(`CELL_${code}`));
      if(c.faultCode&&['MODE','SAFETY_ACK','ROBOT_READY','AIR_CMD','AIR'].includes(c.faultCode)){c.faultCode=null;c.faultText='';}
      if(elapsed>450)setCellStep(2);break;
    }
    case 2:
    ...
    case 4:
      ...
      if(!c.interfaceRelease)setCellFault('INTERFACE','Sequence waiting — ARG2 robot release missing.');
        ...
        const robotReason=robotStartInhibitReason(r);
        if(robotReason)setCellFault('ROBOT_READY',`Sequence waiting — ${c.robot}: ${robotReason}.`);
        else {if(c.faultCode==='INTERFACE')clearCellFault('INTERFACE');if(c.faultCode==='ROBOT_READY')clearCellFault('ROBOT_READY');clearAlarm('CELL_INTERFACE');clearAlarm('CELL_ROBOT_READY');if(elapsed>600)setCellStep(5);}
      }
      break;
    case 5:
...
function loadRandomChallenge(){
  const c=cellState();restoreTrainingCell(false);const choices=['part','clamp','interface','air'];const cause=choices[Math.floor(Math.random()*choices.length)];
  ...
  if(cause==='air')c.airPressureOk=false;
...
function restoreTrainingCell(showToast=true){
  const c=cellState(),r=trainingRobot();Object.assign(c,{safetyDoorClosed:true,safetyAcknowledged:true,partPresent:true,clampCommand:false,robotRelease:false,interfaceRelease:true,failClampFeedback:false,airPressureOk:true,cycleActive:false,sequenceStep:1,stepChangedAt:Date.now(),cycleStartedAt:0,faultCode:null,faultText:'',challenge:null,incorrectActions:0});
...
function renderAdminCellPanel(){
  ...
  const activeCode=c.faultCode||'';
  const tag=(injected,code,step)=>!injected?'NORMAL':activeCode===code?'ACTIVE FAULT':`INJECTED · CHECK ${step}`;
  const injected=[];
  if(!c.safetyDoorClosed)injected.push('Safety door OPEN');
  if(!c.partPresent)injected.push('Part Present = 0');
  if(c.failClampFeedback)injected.push('MM12 feedback failure');
  if(!c.interfaceRelease)injected.push('ARG2 release = 0');
  if(!c.airPressureOk)injected.push('Air pressure feedback = 0');
  const activeText=c.faultText?`ACTIVE FAULT: ${c.faultText}`:'ACTIVE FAULT: none';
  const injectText=injected.length?`INJECTED CONDITIONS: ${injected.join(' · ')}`:'INJECTED CONDITIONS: none';
  el.innerHTML=`<div class="section-title">115020 V01 · fault injection / interactive training cell</div><div class="trainer-help">Green = normal condition. Red = simulated condition injected. An injected condition becomes an active machine fault only when the PLC logic reaches the point that requires it.</div><div class="trainer-grid"><button class="hmi-btn ${c.safetyDoorClosed?'active-green':'red'}" data-action="trainer-safety">Safety door<br><b>${c.safetyDoorClosed?'CLOSED':'OPEN'}</b><small>${c.safetyDoorClosed?'NORMAL':'ACTIVE FAULT · IMMEDIATE'}</small></button><button class="hmi-btn ${c.partPresent?'active-green':'red'}" data-action="trainer-part">Part sensor<br><b>${c.partPresent?'PRESENT':'MISSING'}</b><small>${tag(!c.partPresent,'PART','S002')}</small></button><button class="hmi-btn ${!c.failClampFeedback?'active-green':'red'}" data-action="trainer-clampfault">MM12 feedback<br><b>${c.failClampFeedback?'FAILED':'OK'}</b><small>${tag(c.failClampFeedback,'CLAMP','S003')}</small></button><button class="hmi-btn ${c.interfaceRelease?'active-green':'red'}" data-action="trainer-interface">ARG2 release<br><b>${c.interfaceRelease?'OK':'MISSING'}</b><small>${tag(!c.interfaceRelease,'INTERFACE','S004')}</small></button><button class="hmi-btn ${c.airPressureOk?'active-green':'red'}" data-action="trainer-air">Air pressure feedback<br><b>${c.airPressureOk?'OK':'LOW'}</b><small>${tag(!c.airPressureOk,'AIR','S001')}</small></button><button class="hmi-btn yellow" data-action="trainer-random">Random challenge<br><small>hidden root cause</small></button><button class="hmi-btn" data-action="trainer-restore">Restore cell<br><small>clear injected conditions</small></button></div><div class="trainer-injection ${injected.length?'has-injection':''}">${injectText}</div><div class="trainer-active-fault ${c.faultText?'has-fault':''}">${activeText}</div><div class="trainer-status">Sequence S${String(c.sequenceStep).padStart(3,'0')} · Cycle ${c.cycleActive?'RUNNING':'STOPPED'} · Robot ${r.fault?'FAULT':r.auto?'AUTO READY':'NOT READY'} · Cycles ${c.cycleCount}${c.challenge?` · Challenge ${c.challenge.solved?'SOLVED':'ACTIVE'}`:''}</div>`;
}
...
function updateStationToolbar(){
  ...
  const ack=document.querySelector('[data-action="cell-safety-ack"]');if(ack)ack.classList.toggle('active-green',c.safetyAcknowledged&&c.safetyDoorClosed);const k=document.getElementById('stationK100');if(k){const made=k100Active();k.classList.toggle('active-green',made);k.textContent=made?'K100':'K100';k.title=made?'K100 safety circuit made':`K100 not made — ${k100InhibitReason()}`;}
}
function renderStationStatus(){
  const el=document.getElementById('stationStatusTable');if(!el)return;const ccode=stationCode();
  if(isTrainingStation(state.selectedStation)){const c=cellState(),r=trainingRobot(),events=c.eventLog.slice(0,3);el.innerHTML=`<div class="section-title">Live PLC status</div><div><b>K100</b> ${k100Active()?'1':'0'} · <b>Safety</b> ${c.safetyDoorClosed&&c.safetyAcknowledged?'1':'0'} · <b>Air Cmd</b> ${state.media.air11?'1':'0'} · <b>Pressure</b> ${c.airPressureOk?'1':'0'}</div><div><b>Part</b> ${c.partPresent?'1':'0'} · <b>Clamp cmd</b> ${c.clampCommand?'1':'0'} · <b>MM12b</b> ${state.fixture.MM12b?'1':'0'}</div><div><b>Robot release</b> ${c.robotRelease?'1':'0'} · <b>Auto</b> ${r.auto?'1':'0'} · <b>FK</b> ${r.fk?'1':'0'}</div>${events.map(e=>`<div class="event-mini">${new Date(e.time).toLocaleTimeString()} ${e.text}</div>`).join('')}`;return;}
...
function renderViewer(){
  const el=document.getElementById('viewerSteps');if(!el)return;const c=cellState(),viewerStation=state.viewerStation||state.selectedStation,cellMode=isTrainingStation(viewerStation);const steps=cellMode?Object.keys(CELL_STEPS).map(n=>`S${String(n).padStart(3,'0')}  ${CELL_STEPS[n][0]}`):['S1  S_Init: Grundstellung','T1  T_FRG1: Release robot Drop','S2  S_FRG1: Release','T2  T_M12: Clamp confirmed','S3  S_MM11V: Position','T3  T_M14: Locator','S4  S_FRG2: Robot clear','T4  T_FK: Complete','S5  S_End'];const active=cellMode?c.sequenceStep:state.seqStep;
  const context=document.getElementById('viewerContext');if(context)context.textContent=cellMode?`${viewerStation.replace(' ','')}#AS_DB · LIVE PLC SEQUENCE · S${String(active).padStart(3,'0')}${c.faultText?' · HOLD':''}`:`${viewerStation.replace(' ','')}#AS_DB`;
  el.innerHTML=steps.map((x,i)=>`<button class="viewer-step ${i===active-1?(cellMode&&c.faultText?'active-fault':'active-green'):i<active-1&&cellMode?'completed-step':''}" data-action="viewer-step" data-step="${i+1}">${x}${i===active-1&&cellMode?'  ◀ ACTIVE':''}</button>`).join('');
  const titleEl=document.getElementById('viewerTransitionTitle'),logic=document.getElementById('viewerLogic');if(cellMode&&titleEl&&logic){const s=c.sequenceStep,r=trainingRobot();titleEl.textContent=`S${String(s).padStart(3,'0')}: ${CELL_STEPS[s][1]}${c.faultText?' — HOLD':''}`;const rows={1:[[k100Active(),'K100 Safety Circuit'],[c.safetyDoorClosed&&c.safetyAcknowledged,'Safety'],[(state.stationMode[c.station]||'automatic')==='automatic','Auto mode'],[state.media.air11,'Compressed Air Enable'],[c.airPressureOk,'Air Pressure Feedback']],2:[[c.partPresent,'BGT11a Part Present']],3:[[c.clampCommand,'Clamp Close command'],[state.fixture.MM12b,'MM12b Closed feedback']],4:[[c.interfaceRelease,'ARG2 Interface release'],[robotReadyForAuto(r)&&r.auto,'Robot ready in Auto']],5:[[r.running,'Robot running']],6:[[r.fk,'Robot complete FK']],7:[[!c.clampCommand,'Clamp close command removed']],8:[[!c.cycleActive,'Cycle complete']]}[s]||[];logic.innerHTML=rows.map(([ok,label])=>`<span class="viewer-condition ${ok?'ok':'wait'}">${ok?'1':'0'} · ${label} · ${ok?'TRUE':'WAITING'}</span>`).join('')||'Training logic';}
}
...
function renderMaintenanceActions(){
  const el=document.getElementById('maintenanceActions');if(!el)return;const c=cellState();
  const rows=[
    ['air','Air supply / pressure feedback',c.airPressureOk,'Restore air supply'],
    ['clamp','MM12 clamp feedback device',!c.failClampFeedback,'Restore MM12 sensor'],
    ['part','Part-present sensor BGT11a',c.partPresent,'Restore part sensor'],
    ['interface','ARG2 interface release',c.interfaceRelease,'Restore ARG2 interface'],
    ['safety','Safety door / guard condition',c.safetyDoorClosed,'Close safety door']
  ];
  el.innerHTML=`<div class="section-title">115020 V01 · simulated maintenance actions</div><div class="maintenance-help">Instructor faults are injected in Administration. Use these actions to simulate the physical repair. HMI commands such as Compressed Air Enable remain on their normal HMI pages.</div>${rows.map(([kind,label,ok,action])=>`<div class="repair-row"><span>${label}</span><b class="${ok?'repair-ok':'repair-bad'}">${ok?'OK':'FAULT'}</b><button class="hmi-btn" data-action="maintenance-repair" data-repair="${kind}" ${ok?'disabled':''}>${ok?'No action required':action}</button></div>`).join('')}`;
}
function renderAlarms(){reconcileCurrentAlarms();const list=document.getElementById('alarmList');if(!list)return;let rows;if(state.alarmView==='current')rows=state.alarms.filter(a=>a.active);else rows=state.alarms;list.innerHTML=(rows.length?rows:[{severity:'warning',active:false,ack:true,time:Date.now(),text:'TRAINING: No active alarms.'}]).map(a=>{const d=new Date(a.time||Date.now());const status=a.active?(a.ack?'A':'K'):'G';return `<div class="alarm-line ${a.severity==='fault'&&a.active?'red-line':'yellow-line'}"><span>${d.toLocaleTimeString()}</span><span>${d.toLocaleDateString()}</span><span>${status}</span><span>${a.text}</span></div>`}).join('');}
...
function initScreen(name){
  if(name==='plant'){
    ...
    document.querySelectorAll('.robot-icon').forEach(el=>{const id=(el.dataset.device||'').replace('Robot ','');const r=robots[id];const circle=el.querySelector('circle');const tag=el.querySelector('rect');if(r){const color=robotColor(r);if(circle)circle.setAttribute('class',color==='red'?'robot-red':color==='green'?'robot-green':color==='lightblue'?'robot-blue':'robot-yellow');if(tag)tag.setAttribute('class',color==='red'?'tag-red':color==='lightblue'?'tag-blue':'tag-yellow');}el.addEventListener('click',()=>{if(robots[id]){state.selectedRobot=id;state.selectedStation=robots[id].station;render('robot');}});});
  }
  if(name==='preselection'){document.querySelectorAll('.mode-panel').forEach(el=>el.innerHTML=makeModePanel(el.dataset.cell));const airFb=document.getElementById('air11Feedback');if(airFb)airFb.textContent=`Pressure feedback: ${cellState().airPressureOk?'OK':'LOW'}`;const cv=document.getElementById('controlVoltage');if(cv){const made=k100Active();cv.textContent=made?'On':'Off';cv.classList.toggle('active-green',made);cv.dataset.action='k100-status';cv.title=made?'K100 safety circuit made':`K100 not made — ${k100InhibitReason()}`;}document.querySelectorAll('.utility-card .segmented').forEach(g=>g.querySelectorAll('button').forEach(b=>b.dataset.action='segmented-choice'));}
  ...
  if(name==='maintenance'){
    const el=document.getElementById('maintenanceRows');
    if(el)el.innerHTML=['115000','115020','115050','135100'].map(id=>`<div class="maintenance-row"><div class="maint-station">${id}</div><button class="hmi-btn" data-action="maint-supervisor" data-id="${id}">system<br>supervisor</button><div class="maint-request">${id==='115020'?'Training cell':'Parts request'}</div><button class="hmi-btn ${state.maintenance[id]?'active-green':''}" data-action="maint-call" data-id="${id}">${state.maintenance[id]?'CALL ACTIVE':'Call'}</button></div>`).join('');
    renderMaintenanceActions();
  }
}

function handleAction(button){const action=button.dataset.action;
  if(action==='station'){state.selectedStation=button.dataset.station;state.viewerStation=state.selectedStation;render('station');return;}
  if(action==='station-prev'||action==='station-next'){const i=stations.indexOf(state.selectedStation),d=action==='station-prev'?-1:1;state.selectedStation=stations[(i+d+stations.length)%stations.length];state.viewerStation=state.selectedStation;render('station');return;}
  if(action==='station-open'){state.selectedStation=button.dataset.station;state.viewerStation=state.selectedStation;render('station');return;}
  ...
  if(action==='k100-status'){toast(k100Active()?'K100 safety circuit made':`K100 not made — ${k100InhibitReason()}`);return;}
  if(action==='media-toggle'){const key=button.dataset.media;state.media[key]=!state.media[key];button.classList.toggle('active-green',state.media[key]);button.textContent=state.media[key]?(key.startsWith('air')?'Air':'On'):'Off';if(key==='air11'){if(!state.media[key])setCellFault('AIR_CMD','Sequence waiting — compressed air enable is OFF.');else{clearAlarm('CELL_AIR_CMD');if(state.trainingCell.faultCode==='AIR_CMD'){state.trainingCell.faultCode=null;state.trainingCell.faultText='';}}}else if(!state.media[key])addAlarm(`TRAINING: ${key} media preselection OFF.`,'warning');saveSim();refreshCurrentScreen();return;}
  ...
  if(action==='station-viewer'){state.viewerStation=state.selectedStation;render('viewer');return;} if(action==='fixture-view'){render('fixture');return;} if(action==='cell-interface'){render('interface');return;}
  ...
  if(action==='robot-auto'){const row=button.closest('.robot-row'),id=row.dataset.robot,r=robots[id],reason=robotAutoInhibitReason(r);if(reason){addAlarm(`TRAINING: ${id} Auto rejected — ${reason}.`,'warning',`ROBOT_${id}_AUTO_REJECT`);toast(`Auto inhibited — ${reason}`);renderRobotRows();return;}clearAlarm(`ROBOT_${id}_AUTO_REJECT`);r.auto=!r.auto;if(!r.auto)r.running=false;saveSim();renderRobotRows();refreshCurrentScreen();return;}
  if(action==='robot-start'){const id=button.closest('.robot-row').dataset.robot,r=robots[id],reason=robotStartInhibitReason(r);if(reason){addAlarm(`TRAINING: ${id} Start rejected — ${reason}.`,'warning',`ROBOT_${id}_START_REJECT`);toast(`Start inhibited — ${reason}`);renderRobotRows();return;}clearAlarm(`ROBOT_${id}_START_REJECT`);state.alarms.forEach(a=>{if(a.active&&((a.key===`ROBOT_${id}_STOP`)||(a.text||'').includes(`${id} stopped`))){a.active=false;a.ack=true;a.cleared=Date.now();}});r.running=true;r.fk=true;toast(`${id}: Start accepted`);saveSim();renderRobotRows();refreshCurrentScreen();return;}
  if(action==='robot-stop'){const id=button.closest('.robot-row').dataset.robot;robots[id].running=false;robots[id].fk=false;addHistoryEvent(`TRAINING: ${id} stopped.`,'warning',`ROBOT_${id}_STOP`);saveSim();renderRobotRows();return;}
  ...
  if(action==='robot-ack'){acknowledge();return;} if(action==='robot-correction'){render('correction');return;} if(action==='robot-archive'){toast('Robot archive training view');return;} if(action==='sequence-open'){state.viewerStation=state.selectedRobot===state.trainingCell.robot?state.trainingCell.station:(robots[state.selectedRobot]?.station||state.selectedStation);render('viewer');return;}
  ...
  if(action==='trainer-part'){state.trainingCell.partPresent=!state.trainingCell.partPresent;state.fixture.BGT11a=state.trainingCell.partPresent;if(state.trainingCell.partPresent)clearCellFault('PART');saveSim();renderAdminCellPanel();return;}
  if(action==='trainer-clampfault'){state.trainingCell.failClampFeedback=!state.trainingCell.failClampFeedback;if(!state.trainingCell.failClampFeedback)clearCellFault('CLAMP');saveSim();renderAdminCellPanel();return;}
  if(action==='trainer-interface'){state.trainingCell.interfaceRelease=!state.trainingCell.interfaceRelease;if(state.trainingCell.interfaceRelease)clearCellFault('INTERFACE');saveSim();renderAdminCellPanel();return;}
  if(action==='trainer-air'){state.trainingCell.airPressureOk=!state.trainingCell.airPressureOk;if(state.trainingCell.airPressureOk)clearCellFault('AIR');saveSim();renderAdminCellPanel();return;}
  ...
  if(action==='launch-tia'||action==='launch-eplan'){toast('External engineering application intentionally not launched in training');return;} if(action==='project-info'){const e=document.getElementById('adminInfo');if(e)e.textContent='Project: HMI Training Simulator v1.0.1.4 · synchronized sequence diagnostics · interactive simulated PLC cell · no PLC connection.';return;} if(action==='clean-screen'){toast('Clean screen simulated');return;} if(action==='end-wincc'){toast('Exit disabled in training');return;}
  ...
  if(action==='maintenance-repair'){
    const c=cellState(),kind=button.dataset.repair;
    if(kind==='air'){c.airPressureOk=true;recordEvent('Maintenance restored air pressure feedback');if(c.faultCode==='AIR')clearCellFault('AIR');toast(state.media.air11?'Air supply restored':'Air supply restored — HMI Air Enable still OFF');}
    if(kind==='clamp'){c.failClampFeedback=false;recordEvent('Maintenance restored MM12 feedback device');toast('MM12 feedback device restored');}
    if(kind==='part'){c.partPresent=true;state.fixture.BGT11a=true;clearCellFault('PART');recordEvent('Maintenance restored part-present sensor');toast('Part-present sensor restored');}
    if(kind==='interface'){c.interfaceRelease=true;clearCellFault('INTERFACE');recordEvent('Maintenance restored ARG2 interface release');toast('ARG2 interface restored');}
    if(kind==='safety'){if(!c.safetyDoorClosed)closeSafetyDoor();else toast(c.safetyAcknowledged?'Safety door already healthy':'Safety door closed — acknowledge safety');}
    saveSim();renderMaintenanceActions();refreshCurrentScreen();return;
  }
