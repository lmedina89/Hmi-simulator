...
function k100Active(){const c=cellState(),r=trainingRobot();return !!(c.safetyDoorClosed&&c.safetyAcknowledged&&r.masi&&!r.maintenance);}
...
function trainingPrerequisiteFailure(){
  ...
  if(!k100Active())return 'K100 safety circuit not made';
...
function simulationTick(){
  ...
  switch(c.sequenceStep){
    case 1:{
      ...
      if(!robotReadyForAuto(r)||!r.auto){setCellFault('ROBOT_READY',`Sequence waiting — ${c.robot} not ready in Auto.`);return;}
...
function handleAction(button){const action=button.dataset.action;
  ...
  if(action==='k100-status'){toast(k100Active()?'K100 safety circuit made':'K100 waiting for safety conditions');return;}
  ...
  if(action==='robot-auto'){const row=button.closest('.robot-row'),id=row.dataset.robot,r=robots[id];if(!robotReadyForAuto(r)){addAlarm(`TRAINING: ${id} Auto rejected — robot must be fault-free, PF0/Home reached, MaSi safe, Drives ON, and Maintenance OFF.`,'warning',`ROBOT_${id}_AUTO_REJECT`);toast('Auto inhibited');renderRobotRows();return;}clearAlarm(`ROBOT_${id}_AUTO_REJECT`);r.auto=!r.auto;if(!r.auto)r.running=false;saveSim();renderRobotRows();refreshCurrentScreen();return;}
  if(action==='robot-start'){const id=button.closest('.robot-row').dataset.robot,r=robots[id];if(!robotReadyForAuto(r)||!r.auto){addAlarm(`TRAINING: ${id} Start rejected — robot must be fault-free, homed, safe, Drives ON, Auto selected, and Maintenance OFF.`,'warning',`ROBOT_${id}_START_REJECT`);toast('Start inhibited');renderRobotRows();return;}clearAlarm(`ROBOT_${id}_START_REJECT`);state.alarms.forEach(a=>{if(a.active&&((a.key===`ROBOT_${id}_STOP`)||(a.text||'').includes(`${id} stopped`))){a.active=false;a.ack=true;a.cleared=Date.now();}});r.running=true;r.fk=true;toast(`${id}: Start accepted`);saveSim();renderRobotRows();refreshCurrentScreen();return;}
  ...
  if(action==='launch-tia'||action==='launch-eplan'){toast('External engineering application intentionally not launched in training');return;} if(action==='project-info'){const e=document.getElementById('adminInfo');if(e)e.textContent='Project: HMI Training Simulator v1.0.1.3 · synchronized sequence diagnostics · interactive simulated PLC cell · no PLC connection.';return;} if(action==='clean-screen'){toast('Clean screen simulated');return;} if(action==='end-wincc'){toast('Exit disabled in training');return;}
